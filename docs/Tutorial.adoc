...
=== Raspberry Pi Camera
Die Raspberry Pi Camera ist ein Device, welches standardmässig unterstützt wird und auch in Rasbian vier Applikationen enthält, welche man vom Terminal aus verwenden kann.

* `raspistill` und `raspiyuv` für die Bildaufnahme
* `raspivid` und `raspividyuv` für die Videoaufnahme

`raspiyuv` und `raspividyuv` verwenden keinen Encoder, sondern senden ihr YUV (Ein Farbmodell wie RBG) direkt von der Kamera an ein File. Diese beiden Applikationen werden nicht unterstützt in der Java-Device Klasse.
