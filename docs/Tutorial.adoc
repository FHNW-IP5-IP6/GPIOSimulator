...
Der Grove Base Hat stellt Adapter für PWM, Digital, Analog, UART sowie I2C zur Verfügung. Zusätzlich werden alle GPIO Pins auf den Hat projiziert und können mittels Jumper Kabel weiterverwendet werden. Die meisten von uns implementierten Komponenten unterstützten beide Anschlussarten.
...
==== PWM
PWM steht für **P**ulse **W**idth **M**odulation. Der Grove Base Hat unterstütz die Hardware PWM Pins 12 sowie 13. Beide Pins sind im selben Adapter vereinigt.

==== UART
Der UART Adapter macht den Serial Bus der Raspberry Pi für Grove Sensoren und Aktuatoren verfügbar. Er benutzt die GPIO Pins 14 und 15. An diesem Port würden wir die Grove Serial Camera anschliessen, wenn diese mit 3.3 Volt kompatibel wäre. Leider benötigt die Kamera aber eine Spannungsquelle mit 5V, weshalb das Kamera Modul manuell verkabelt werden muss.
...
=== Pi4J
Die Software Bibliothek Pi4J bietet vollzugriff auf die I/O Funktionalität des Raspberry Pi. Die Library abstrahiert die komplexe Hardware Programmierung und bietet stattdessen einfache API-Calls an, welche direkt verwendet werden können. Folgende Funktionalität wird von Pi4J abgedeckt: <<Pi4J>>
...
=== Log4j 2
Wir benutzen Log4j 2 von Apache für das Schreiben von Logs in ein Rolling File. Log4j 2 ist ein beliebter und weit verbreiteter Vertreter der Java Logging Frameworks. <<Log4j>>
...
==== Gradle
Gradle wird für das Bauen des Codes benötigt. Mittels dem `wget` Befehl kann das Build Tool direkt von der Herstellerseite heruntergeladen werden. Wir haben die Version 6.5.1 verwendet. <<GradleInstall>>
...
Auf dem Pi selbst müssen Sie dann nur noch ihr Projekt bauen und ausführen. Während der Entwicklung geniessen Sie allerdings alle Vorzüge der IDE auf ihrem Rechner.
...
== Pi4J Basics
Wie wir im Tutorial bereits erläutert haben, verwenden unsere Component Klassen die Library Pi4J. Eine Vielzahl der von uns zur Verfügung gestellten Konstruktoren benötigen als Input verschiedene Typen von Pi4J GPIO Pins. In diesem Kapitel möchten wir erklären, wie solche Pins via Pi4J angefordert werden können.

=== GPIOProvider
Zu Beginn eines jeden Pi4J GPIO Projektes muss der gewünschte Standard Provider gesetzt werden. In unseren Beispielen verwenden wir ausschliesslich den `RaspiGpioProvider` mit dem Numbering Schema `RaspiPinNumberingScheme.BROADCOM_PIN_NUMBERING`.
...
Das `RaspiPinNumberingScheme.BROADCOM_PIN_NUMBERING` entspricht den Nummern auf dem unteren Pinout.
...
Die verschiedenen Pins können wie folgend vom GpioController angefordert werden:
