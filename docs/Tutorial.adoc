...
Darüber hinaus hat die Raspberry Pi die Fähigkeit, mit der Außenwelt zu interagieren. An den sogenannten GPIO (General Purpose Input Output) Pins kann man diverse Sensoren und Aktoren anschliessen, was den Raspberry Pi sehr beliebt für IoT-Projekte macht.
...
source: https://www.raspberrypi.org/help/what-%20is-a-raspberry-pi/[RaspberryPi.org]
        https://www.elektronik-kompendium.de/sites/raspberry-pi/2002191.htm[Elektronik-Kompendium]
...
Grove erleichtert das Anschließen und Experimentieren mit den Geräte. Die Sensoren und Aktuatoren können mittels der Grove Kabel an einen Grove Aufsatz, wie dem Grove Base Hat angeschlossen werden, somit sind keine Jumper oder Lötkolben mehr erforderlich für einfache IoT Projekte.

src: https://www.seeedstudio.com/category/Sensor-for-Grove-c-24.html [Grove Sensors and actuators]
...
|LedComponent(com.pi4j.util.Console console, com.pi4j.io.gpio.GpioPinDigitalOutput pin) |Constructor for directly controlling digital LEDs without PWM.
|LedComponent(com.pi4j.util.Console console, com.pi4j.io.gpio.GpioPinPwmOutput pwmPin) |Constructor for controlling LEDs connected to pins with hard- or software PWM. This constructor allows controlling the brightness of the LED.
|LedComponent(com.pi4j.util.Console console, GroveAdapter adapter) |Constructor for controlling Grove LEDs.
|===
...
|void |turnOff() |Turns the LED off. If a PWM pin was provided in the constructor, the brightness is set to 0%.
|void |turnOn() |Turns the LED on. If a PWM pin was provided in the constructor, the brightness is set to 100%.
|===
...
|===
|Konstruktor |Beschreibung
|ButtonComponent(com.pi4j.util.Console console, com.pi4j.io.gpio.GpioPinDigitalInput pin) |Constructor for GPIO pins without value reversion.
|ButtonComponent(com.pi4j.util.Console console, com.pi4j.io.gpio.GpioPinDigitalInput pin, boolean reverse) |Constructor for GPIO pins.
|ButtonComponent(com.pi4j.util.Console console, GroveAdapter adapter) |Constructor for digital grove devices without value reversion.
|ButtonComponent(com.pi4j.util.Console console, GroveAdapter adapter, boolean reverse) |Constructor for digital grove devices.
|===
...
|===
|Konstruktor |Beschreibung
|I2CLCD​(int address, int busNumber) |Standard Constructor that needs address and the busnumber to initialise
|I2CLCD​(com.pi4j.io.i2c.I2CDevice device)	
...
|void |displayText​(String text) |displays a text on the first line of the display.
|void |displayText​(String text, int line) |writes string to the lcd display on a specific line.
|void |displayText​(String text, int line, int pos, boolean jumpToNextLine) |displays the string with an additional position.
...
|void |displayBounceText​(String text, int line, int delay, boolean jumpToNextLine, boolean startAgain) |displays the text and scrolls to the sides, but bounces back.
...
|===
|Konstruktor |Beschreibung
|RFIDReaderComponent​(com.pi4j.util.Console console) |Constructor of the RFID Reader
|===
...
[source,java]
----
include::{sourcedir}/fhnwexamples/component/UltraSonicRanger.java[tags=UltraSonicRanger]
----
...
|===
|Konstruktor |Beschreibung
|ServoMotorComponent(com.pi4j.util.Console console, com.pi4j.io.gpio.GpioPinPwmOutput pin) |Constructor with default value for pulseMin = 500, pulseMax = 2400 and maxDegrees = 180.
|ServoMotorComponent(com.pi4j.util.Console console, com.pi4j.io.gpio.GpioPinPwmOutput pin, int pulseMin, int pulseMax) |Constructor with default value for maxDegrees of 180 degrees.
|ServoMotorComponent(com.pi4j.util.Console console, com.pi4j.io.gpio.GpioPinPwmOutput pin, int pulseMin, int pulseMax, int maxDegrees) |Constructor with all possible values.
|===
...
|===
|Konstruktor |Beschreibung
|RaspberryPiCameraComponent​(com.pi4j.util.Console console, uk.co.caprica.picam.CameraConfiguration raspiStillConfiguration) |Constructor for only using the picture functionality
|RaspberryPiCameraComponent​(com.pi4j.util.Console console, fhnwgpio.components.helper.RaspiVidConfiguration raspiVidConfiguration) |Constructor for only using the video functionality
|RaspberryPiCameraComponent​(com.pi4j.util.Console console, uk.co.caprica.picam.CameraConfiguration raspiStillConfiguration, fhnwgpio.components.helper.RaspiVidConfiguration raspiVidConfiguration) |Constructor for using the picture and video functionality
|===
...
[source,java]
----
include::{sourcedir}/fhnwgpio/components/I2CLCD.java[tags=LCD1602Commands]
----
...
[source,java]
----
include::{sourcedir}/fhnwgpio/components/I2CLCD.java[tags=LCDInit]
----
...
[source,java]
----
include::{sourcedir}/fhnwgpio/components/I2CLCD.java[tags=LCDDisplayText]
----
...
[source,java]
----
include::{sourcedir}/fhnwgpio/components/I2CLCD.java[tags=LCDDisplayScrollText]
----
...
[source,java]
----
include::{sourcedir}/fhnwgpio/components/I2CLCD.java[tags=LCDDisplayBounceText]
----
