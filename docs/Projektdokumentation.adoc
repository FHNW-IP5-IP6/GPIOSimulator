...
=== Simulatiosstrategie
Zu Beginn war es unser Plan das Factory Design Pattern zu verwenden, um zwischen dem Simulator und dem richtigen Hardwarezugriff mit Pi4J zu differenzieren. Da der erste Zugriffspunkt in Pi4J immer eine Factory ist, wie z. B die GpioFactory, I2CFactory oder die SerialFactory. 
...
Nachdem wir einige Zeit damit verbracht haben, die Pi4J-Device Simulationsstrategie zu verfolgen, sind wir auf neue Hindernisse gestossen.

Für den Simulator haben wir eine Factory erstellt, welche im Konstruktor einen Boolean annimmt. Dieser Boolean bestimmt dann, welcher Typ von Device zurückgegeben wird. Entweder ein Pi4J Device oder unser Custom LED Simulator Device.
...
----
Das Simulator LED Device unterstützt im Moment lediglich die Funktion blink(), welche wiederum die Funktionen on() und off() aufrufen. In den beiden letzteren Funktionen wird jeweils einfach der aktuelle Status mittels Log4J2 auf die Konsole und in ein File geschrieben.
...
Soweit so gut. Um unsere Implementation zu testen, haben wir die Klasse BlinkLedDevice so modifiziert, dass wir eine LEDBase vom Typ GpioLEDSimulator erhalten und darauf die blink() Funktion ausgeführt.
...
Beim Testen unseres Codes mussten wir allerdings feststellen, dass diverse Pi4J Aufrufe nicht funktionieren, welche für die Hardware Simulation allerdings immer von Nöten sein werden. Beispielsweise ist es uns nicht möglich, über die statische Klasse GpioFactory den Default-Provider zu setzen, wenn wir als Provider den RaspiGpioProvider verwenden. Dieser greift in seinen Tiefen auf WiringPi zu. WiringPi erwartet dann gewisse Files an einem bestimmten Ort auf einem Linux System, welche er auf meinem Windows Rechner natürlich nicht findet.

Eine andere Möglichkeit wäre es, anstelle des RaspiGpioProvider einen anderen Provider zu benutzen. Pi4J stellt nämlich einen SimulatedGpioProvidre zur Verfügung.

[source,java]
----
GpioFactory.setDefaultProvider(new SimulatedGpioProvider());
----
Momentan wissen wir, dass dieser Provider das gleiche Interface implementiert wie der RaspiGpioProvider. Allerdings retourinert er wohl überall einfach null oder löst bestimmte Events nicht aus. Wenn wir diesen Provider in einem unserer GPIO Beispiele verwenden, passiert einfach nichts. Bei der Anwendung in unserem LED Simulator Device wird aber wie gewünscht geloggt.
So sieht ein beispiel aus:

Uns bleiben daher 2 Möglichkeiten, das Projekt weiterzuführen

==== SimulatedGpioProvider verwerden
Generell gehen wir davon aus, dass wir unter Verwendung des des SimulatedGpioProvider den Simulator wie geplant implementieren können. Der Benutzer müsste seinen Code jedoch nicht nur an einer Stelle anpassen, sondern an zwei. Das wäre aber wohl noch verkraftbar.
Durch unsere bisherig gesammelten Erfahrungen mit den Sensoren und Aktuatoren sind wir uns aber unsicher, wie viel eine Simulation dem Studenten überhaupt bringt. Wenn wir beispielsweise ein I2C LCD simulieren würden, könnten wir ja lediglich den Text an einem bestimmten Ort (Konsole/ File /Simulator) weitergeben und anzeigen. Ob nun die Verkabelung stimmt und der Text in die richtigen Bytes zerlegt wird, können wir wohl kaum testen/ sicherstellen.


==== Der Simulator wird zum ausführlichen Tutorial
Diesen Ansatz haben wir ja bereits besprochen. Resultat der Projektarbeit wären dann eine Art Guide für die Verwendung von Pi4J Devices, welche wir mit unseren eigen Devices ergänzen können. Zusätzlich können wir noch die Grove Adapter unterstützen. Wir bieten dem Studenten so stabile Beispiele, welche auf der verwendeten Hardware funktionieren. Unserer Meinung nach wäre dieser Ansatz wesentlich gewinnbringender für die Studenten, da eine oberflächliche Simulation keinen Nutzen hat, weil die schwierige Arbeit immer von der tatsächlichen Hardware abhängen wird.

==== Entscheidung
Gemeinsam mit unseren Coaches haben wir uns dafür entschieden ein ausführliches Tutorial zu schreiben, da es schlussendlich erfolgsversprechender ist und einen grösseren Nutzen hat für die Studenten.

Das Tutorial soll ein separates Dokument sein und am Anfang auch einen Setup enthalten. Man kann auch ein Image für die angehenden Studenten zur Verfügung stellen, um dies zu erleichtern.

Bei den Examples muss ausserdem darauf geachtet werden, dass sie nicht zu komplexe Themen wie Asynchronizität ansprechen, da dass den Rahmen eines Projektes für das erste und zweite Semester sprengt.

Wir können auch mehrere Beispiele machen mit verschiedenen Schwierigkeitsgraden, was zu den Fähigkeiten der Studenten passt. z.B eines was nur in einer Schleife funktioniert für den Anfang und ein weiteres mit.

Beispiel Source code Einbindung für die Zukunft:
[source,java]
----
include::{sourcedir}/FHNWExamples/gpio/BlinkLed.java[lines=5..10]
----
