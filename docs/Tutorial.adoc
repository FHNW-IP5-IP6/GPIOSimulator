...
== Komponenten
In diesem Kapitel sind einige der gängigen Sensoren und Aktuatoren beschrieben und wie diese an den Raspberry Pi angeschlossen werden können. Ausserdem wird beschrieben, wie die Geräte mit den Komponentenklassen in Java verwendet werden können
...
==== Anwendungsbeispiel
Die blinkende LED ist das "Hello World"-Äquivalent der GPIO Programmierung. Dieses Beispiel lässt eine LED zwei Mal blinken. Anschliessend wird die Helligkeit der LED schrittweise von 0% auf 100% inkrementiert. Nach dem Erreichen der maximalen Helligkeit dekrementiert die Lichtstärke wieder auf 0%. Das Dimmen der LED ist durch «Pulse Width Modulation» möglich. Diese Funktionalität wird durch die Übergabe eines `GpioPinPwmOutput` Pins an den Konstruktor der `LedComponent` ermöglicht.
...
*Achtung:* Die Buzzer-Komponente manipuliert die PWM Grundfrequenz des Raspberry Pi beim Abspielen von Tönen sehr stark. Der Buzzer kann deshalb nicht zusammen mit anderen PWM Devices verwendet werden. Ist dies für ein Projekt nötig, muss auf einen externen PWM Generator zugegriffen werden.
...
==== Anwendungsbeispiel
In diesem Code Beispiel wird ein Text, welcher vom Nutzer eingegeben werden kann, in der ersten Zeile des LCD-Displays angezeigt und die Uhrzeit in der Zweiten. Die Uhrzeit wird im Sekundentakt aktualisiert. +
...
=== Grove RFID Reader
Der Grove RFID Reader ist ein 125KHz RFID Reader, welches em4100 RFID Karteninformationen seriell lesen kann. Die maximale Distanz zum Lesen beträgt 70mm. Die RFID Reader benötigt eine Spannung von 5 Volt, weshalb es nicht mit dem Grove Hat kompatibel ist, da dieser nur 3.3 Volt liefert. Es ist jedoch möglich es mit Kabeln direkt an dem Pi zu verbinden, wie bei der Grove Serial Camera. <<RfidReader>>
...
*Achtung:* Um Code auszuführen, der auf die seriellen Ports zugreift, muss man es als Superuser ausführen z. B: `sudo java -jar Example.jar`
...
=== Ultrasonic Ranger
Der Grove Ultrasonic Ranger ist ein Grove Modul, welches an eines der digitalen Adapter des Grove Hats angeschlossen werden kann. Der Ultrasonic Ranger misst die Distanz zwischen dem Sensor und einem Ziel indem die Zeit zwischen der Emission und der Rezeption einer Ultraschallwelle gemessen wird. Es kann Distanzen von bis zu 2m lesen. <<UltrasonicRanger>>
...
==== Anwendungsbeispiel
Das unten abgebildete Beispiel zeigt die Verwendung der `ServoMotorComponent`. Es benutzt den Konstruktor mit den meisten Standardwerten. Dieser eignet sich für die meisten 180° Mini und Micro Servos.
...
==== Anwendungsbeispiel
Das unten Code Beispiel zeigt, wie mit der Hilfe der `SerialCameraComponent` ein JPEG Foto geschossen und unter einem bestimmten Pfad abgespeichert werden kann.
...
=== Raspberry Pi Camera
Die Raspberry Pi Camera ist die Kamera Komponente, welche standardmässig vom Pi unterstützt wird. Es ist eine kleine 8MP Kamera, die über ein Flachbandkabel direkt an dem Pi angeschlossen werden kann. Man kann maximal 3280 x 2464 Pixel für Bilder verwenden und für Videos 1080p mit 30fps oder 720p mit 60fps oder 640x480p mit 90fps.
...
Für die Verwendung muss die Kamera zunächst im OS aktiviert werden. Unter den `Interfacing Options`, von `raspi-config` kann es problemlos aktiviert werden.
...
==== Anwendungsbeispiel
In diesem Code Beispiel wird die Raspberry Pi Camera mit der `RaspberryPiCameraComponent` verwendet, um zwei Bilder und ein Video aufzunehmen im Ordner `/home/pi/Pictures/`.  +
...
Die 12V Spannungsquelle muss mittels Schraubendreher am Treiber befestigt werden. Befestigungsstelle ist im unteren Bild rot umrahmt. Das Grove Kabel, welches mit einem digitalen Output des Grove Base Hat verbunden ist, muss an den Grove Anschluss _IN_ des Treibes angeschlossen werden. Die Kabel zur LED müssen ebenfalls mit dem Schaubendreher am Treiber angeklemmt werden. Die einzelnen Verbindungen sind mit G, R, B, + beschriftet. Diese müssen entsprechend mit dem LED Band verbunden werden.
...
== Beispielprojekt
Dieses Beispielprojekt soll zeigen, wie man die Component Klassen in ein neues Projekt einbindet und wie die Geräte in einem kleinen Projekt zusammenspielen können.

Das Projekt ist ein Photo Booth, also eine Fotobox in der die folgenden Geräte zusammenarbeiten müssen, um ein gutes Foto von einer Person zu machen:
...
=== Setup
In den folgenden Unterkapiteln wird gezeigt wie ein neues GPIO Projekt aufgesetzt werden kann und welche Schritte nötig sind, um die Component Klassen zusammen mit Pi4J nutzen zu können.

==== Component Klassen als Library hinzufügen
Um die Component Klassen in einem neuen Projekt zu verwenden, muss man die Jar-Datei in dem neuen Projekt als Bibliothek hinzufügen.
...
In der Datei `build.gradle` muss dann nur noch die Library angeben, dann sind die Komponenten Klassen wie in den Beispielen in Kapitel 5 nutzbar:
...
Die Component Klassen können Informationen und eventuelle Fehler die auftreten könnten in Logs aufzeichnen. Diese Logs können im Beispielprojekt gesteuert werden, dafür muss zunächst log4j2 unter den gradle dependencies hinzugefügt werden im `build.gradle` File:
...
Danach wird eine neue Ressource benötigt, welche dann die Logs steuert, die Datei `log4j2.xml`. Alle Logs der Component Klassen werden unter dem Namen `ComponentLogger` geloggt.
...
Für mehr Informationen über Log4J2 und der Konfigurationsdatei findet man über folgendem Link: https://logging.apache.org/log4j/2.x/manual/configuration.html
...
In einem ersten Schritt haben wir uns überlegt, welche Zustände die Photo Booth überhaut haben kann und wie diese Zustände miteinander agieren sollen. Diese sind in folgendem State Diagramm festgehalten.
...
Zuerst werden alle benötigten Komponentenklassen initialisiert, um sie in den States verwenden zu können:
...
Der State 0 wurde über die Methode `isButtonPressed()` realisiert. Zunächst wird auf dem LCD-Display mitgeteilt, dass der Ablauf erst beginnt, wenn der Button gedrückt wird. Sobald dieser gedrückt wird, gibt die Methode `true` als Rückgabewert zurück, damit zum nächsten State fortgefahren werden kann.
...
Der State 1 wurde über die Methode `isDistanceInRange()` realisiert. Zu Beginn wird mittels LCD-Display gezeigt, welcher Abstand erwünscht ist. Der LED-Strip wird rot um zu sugerieren, dass der Abstand noch nicht korrekt ist. 
Der Ultrasonic Ranger misst den Abstand sekündlich und sobald der Abstand in Ordnung ist, wird die Farbe des LED-Strips zu grün gewechselt und die Methode gibt `true` zurück, damit mit dem nächsten State fortgefahren werden kann.
...
Der State 2 wurde über die Methode `countdown()` realisiert. Hier wird von der Zahl 5 hinabgezählt. Das Herunterzählen wird mit mehreren Geräten untermalt, um die Spannung zu erhöhen. Das LCD-Display zeigt den Countdown an und der Buzzer macht jeweils einen Buzz-Ton für 200 Millisekunden. Der Steppermotor führt eine Umdrehung von 400 Schritten aus, da es pro Schritt 2 Millisekunden benötigt, braucht es für den Countdown einer Zahl genau eine Sekunde.
...
Der State 3 wurde über die Methode `takePicture()` realisiert. Bevor das Bild geschossen wird, wird der LED-Strip auf weiss gesetzt und der Buzzer spielt einen höheren Ton. Dann macht die Raspberry Pi Kamera das Bild und speichert es in den Bilderpfad des Raspberry Pi unter `/home/pi/Pictures/picam1.jpg`. Dass das Bild geschossen wurde, wird auf dem LCD-Display angezeigt und der LED-Strip wird wieder abgeschaltet.
...
Der gesamte Ablauf wird dann in der `execute()` Methode ersichtlich. Diese Methode ist dafür verantwortlich, dass der Ablauf wieder von vorne beginnt bei State 0, sobald ein Bild gemacht wurde.
