...
=== Setup
In den folgenden Unterkapiteln beschäftigen wir uns mit dem Aufsetzen von neuen GPIO Projekten. Wir haben dokumentiert, welche Schritte nötig sind, um unsere Komponenten Klassen zusammen mit Pi4J nutzen zu können.
...
=== Umsetzung
Das Gradle Projekt ist nun vorbereitet. Wir können nun mit der Implementation unserer Photo Booth starten.

==== Ausarbeitung Projektidee
In einem ersten Schritt haben wir uns überlegt, welche Zustände unsere Photo Booth überhaut haben kann und wie diese Zustände miteinander agieren. Dies haben wir in einem State Diagramm festgehalten.

.State Machine
image::Sample_Project_States.png[width=75% Project State Machine]

Die blauen Ovale im Diagramm entsprechen den einzelnen Zuständen. Mit den Pfeilen wird gezeigt, wie zwischen den Zuständen gewechselt werden kann. Im State 0 wird beispielsweise nur dann zu State 1 gewechselt, wenn ein Button gedrückt wurde. Wird der Button nicht gedrückt, bleiben wir im State 0.

Als nächstes haben wir uns überlegt, was in den jeweiligen States alles geschehen soll.

[.underline]*State 0*
In diesem State warten warten wir auf einen User, welcher die Photo Booth via Button Click aktiviert. Wir zeigen auf dem LCD einen Text an, welchen den Benutzer dazu einlädt, den Knopf zu drücken. Sobald der Button betätigt wurde, wechseln wir in den nächsten State.

[.underline]*State 1*
In diesem State warten wir solange, bis sich der Benutzer korrekt positioniert hat. Wir fordern den Benutzer via LCD auf, sich einen Meter von der Kamera zu entfernen. Die Distanz messen wir mit dem Grove Ultrasonic Ranger und zeigen Sie auf dem Display an. Solange sich der User nicht korrekt platziert hat, lassen wir das LED Band rot leuchten. Sobald der Benutzer sich in der entsprechenden Distanz eingefunden hat, wechseln wir die Farbe des Bandes auf grün und wechseln zu State 2.
...
[.underline]*State 3*
In diesem Status wird das Bild geschossen. Bevor dies passiert, lassen wir den LED Streifen weiss aufleuchten und den Buzzer einen Ton abspielen. Anschliessend wir das Bild erstellt und abgespeichert. Auf dem LCD wird angezeigt, dass das Bild geschossen wurde. Zu guter Letzt wird der Stepper Motor wieder auf seine initiale Position gesetzt und in den State 0 gewechselt.
...
Der State 1 wurde über die Methode `isDistanceInRange()` realisiert. Zu Beginn wird mittels LCD-Display gezeigt, welcher Abstand erwünscht ist. Der LED-Strip wird rot um zu sugerieren, das der Abstand noch nicht korrekt ist. 
...
Der gesamte Ablauf wird dann in der `execute()` Methode ersichtlich. Diese Methode ist dafür verantwortlich, dass der Ablauf wieder von vorne beginnt, sobald ein Bild gemacht wurde.
